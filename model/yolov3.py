...
def YoloV3(input_shape=(416, 416, 3), num_classes=2, training=False):
    # 3 * (4 + 1 + num_classes)
...
class YoloLoss:
    def __init__(self, valid_anchors_wh, num_classes, ignore_thresh=0.5, lambda_coord=5.0, lambda_noobj=0.5):
    ...
    def __call__(self, y_true, y_pred):
        ...
        wh_loss = self.calc_wh_loss(true_wh_rel, pred_wh_rel, true_obj, weight)
        # !!**!! add landmark loss later
        class_loss = self.calc_class_loss(true_class, pred_class, true_obj)
    ...
    def calc_wh_loss(self, true_wh, pred_wh, true_obj, weight):
        wh_loss = tf.reduce_sum(tf.square(true_wh - pred_wh), axis=-1)

        true_obj = tf.squeeze(true_obj, axis=-1)

        wh_loss = wh_loss * true_obj * weight
        wh_loss = tf.reduce_sum(wh_loss, axis=(1, 2, 3)) * self.lambda_coord

        return wh_loss

    def calc_class_loss(self, true_class, pred_class, true_obj):
