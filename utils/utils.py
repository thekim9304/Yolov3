...
def get_absolute_yolo_box(y_pred, valid_anchors_wh, num_classes):
    ...
    """
    box_xy, box_wh, objectness, classes = tf.split(y_pred, (2, 2, 1, num_classes), axis=-1)

    box_xy = tf.sigmoid(box_xy)
    objectness = tf.sigmoid(objectness)
    class_probs = tf.sigmoid(classes)
    ...
    return bbox_abs, objectness, class_probs, bbox_rel


def get_relative_yolo_box(y_true, valid_anchors_wh, num_classes):
    box_xy, box_wh, objectness, classes = tf.split(y_true, (2, 2, 1, num_classes), axis=-1)
    bbox_abs = tf.concat([box_xy, box_wh], axis=-1)
